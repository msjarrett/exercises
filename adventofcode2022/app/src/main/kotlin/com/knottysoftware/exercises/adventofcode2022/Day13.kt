...
class Day13 : Exercise {
  ...
  override fun parse(lines: Sequence<String>) {
    packetPairs =
        lines.chunked(3).map { Pair(parseList(it[0], 0).first, parseList(it[1], 0).first) }.toList()
  }
  ...
  fun isInOrder(left: ListNode, right: ListNode): Boolean? {
    ...
    while (true) {
      ...
      else if (i == left.children.size) return true else if (i == right.children.size) return false
  ...
  override fun partOne(): Int =
      packetPairs.withIndex().sumOf { (index, value) ->

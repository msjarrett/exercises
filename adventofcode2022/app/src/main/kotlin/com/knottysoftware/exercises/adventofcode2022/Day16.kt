...
class Day16 : Exercise {
  ...
  private final val REGEX = Regex("""Valve ([A-Z][A-Z]) has flow rate=(\d+); tunnels? leads? to valves? ([A-Z, ]+)$""")
  private final val TIME_LIMIT = 30
  private final val START_NODE = "AA"
  ...
  override fun parse(lines: Sequence<String>) {
    ...
    this.relevantNodes = buildSet<Node> {
      addAll(pressure.keys)
      add(START_NODE)
    }
  }

  inner class State private constructor(
    val elapsed: Int,
    val released: Int,
    val position: Node,
    val opened: Set<Node>) {
    ...
    val pos: Node get() = position
    val visitedKey: Node get() = position
    ...
    fun openValve(): State {
      ...
      return State(elapsed + 1, released + (TIME_LIMIT - elapsed - 1) * pressure[position]!!, position, buildSet {
        addAll(opened)
        add(position)
      })
    }
  ...
  inner class StateEx private constructor(
    val released: Int,
    val pos1: Node,
    val nextTurn1: Int,
    val pos2: Node,
    ...
    constructor(startTime: Int) : this(0, START_NODE, startTime, START_NODE, startTime, setOf<Node>())

    val pos: Node get() = if (!turn) pos1 else pos2
    val visitedKey: Node get() = if (pos1 < pos2) pos1 + pos2 else pos2 + pos1
    val elapsed: Int get() = if (turn) nextTurn2 else nextTurn1

    private val turn get() = nextTurn2 < nextTurn1

    fun moveTo(node: Node, cost: Int): StateEx = if (!turn) StateEx(released, node, nextTurn1 + cost, pos2, nextTurn2, opened) else StateEx(released, pos1, nextTurn1, node, nextTurn2 + cost, opened)
    
    fun openValve(): StateEx {
      check(!opened.contains(pos))
      check(pressure[pos]!! != 0)
      return StateEx(released + (TIME_LIMIT - elapsed - 1) * pressure[pos]!!, pos1, if (!turn) nextTurn1 + 1 else nextTurn1, pos2, if (turn) nextTurn2 + 1 else nextTurn2, buildSet {
        addAll(opened)
        add(pos)
      })
    }
  ...
  fun generateDistanceMap() = relevantNodes.associateWith {
      ...
      while (!queue.isEmpty()) {
        ...
        visited.put(n, cost)
        queue.addAll(paths[n]!!.map { Pair(it, cost + 1) } )
      }
  ...
  fun partOneByNodeSearch(): Int {
    // A* could be good here, but lets start with BFS.
    ...
    val visited = buildMap<Node, MutableSet<Pair<Int, Set<Node>>>> {
  ...
  fun partOneByGraphSearch(): Int {
    ...
    while (!queue.isEmpty()) {
      ...
      // - We came here before.
      if (visited[state.visitedKey] == null) visited.put(state.visitedKey, mutableSetOf<Pair<Int, Set<Node>>>())
      if (visited[state.visitedKey]!!.contains(Pair(state.released, state.opened))) continue
      ...
      if (!state.opened.contains(pos) && pressure[pos] != null) {
        ...
        for (next in distanceMap[pos]!!.keys.filter { !state.opened.contains(it) }) {
          queue.addLast(state.moveTo(next, distanceMap[pos]!![next]!!))
        }
      }
    } // while (!queue.isEmpty())
    return bestState.released
  ...
  fun partTwoByGraphSearch(): Int {
    ...
    while (!queue.isEmpty()) {
      ...
      if (state.released > bestState.released) {
        bestState = state
        println("T = ${bestState.elapsed}  SCORE = ${bestState.released}  OPENED = " + bestState.opened.size)
      }
      ...
      if (!state.opened.contains(pos) && pressure[pos] != null) {
        // If this is not the starting node and we haven't opened it, open now.
        // We wouldn't come here unless we were going to open it, so it's always the next state.
        queue.addFirst(state.openValve())
      } else {
        // Already open, find nodes we haven't visited.
        for (next in distanceMap[pos]!!.keys.filter { !state.opened.contains(it) }) {
          queue.addFirst(state.moveTo(next, distanceMap[pos]!![next]!!))
        }
      }
    } // while (!queue.isEmpty())
    return bestState.released

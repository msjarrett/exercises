...
class Day2 : Exercise {
    ...
    override fun partOne(): Long {
        return strategies.map { Pair(moveForChar(it.first), moveForChar(it.second)) }
        .map { score(it.first, it.second) }
        .sum().toLong()
    }

    override fun partTwo(): Long {
        return strategies.map { Pair(moveForChar(it.first), resultForChar(it.second)) }
        .map {
        ...
        }.map { score(it.first, it.second) }.sum().toLong()
    }
}
...
class Day12 : Exercise {
  ...
  private fun pathLength(
      start: Point,
      testEnd: (Point) -> Boolean,
      testMove: (Point, Point) -> Boolean
  ): Int {
  ...
  override fun partOne(): Int =
      pathLength(start, { p -> p == end }, { c, p -> map[c.y][c.x] <= (map[p.y][p.x] + 1) })
  override fun partTwo(): Int =
      pathLength(end, { p -> map[p.y][p.x] == 0 }, { c, p -> map[c.y][c.x] >= (map[p.y][p.x] - 1) })
}

...
// Borrowed from
// https://slack-chats.kotlinlang.org/t/454925/any-built-in-way-to-create-a-flow-of-lines-from-a-file-on-jv
...
private fun RunPuzzle(lines: Flow<String>, puzzle: Puzzle): Any {
  println("=== START")
  val result = runBlocking { puzzle(lines) }
...
fun RunPuzzleWithInput(puzzle: Puzzle, year: Int, day: Int) =
    RunPuzzle(File(GetPuzzleInput(year, day).toUri()).lineFlow(), puzzle)

fun RunPuzzleWithText(puzzle: Puzzle, input: String) =
    RunPuzzle(input.trimIndent().split("\n").asFlow(), puzzle)

...
suspend fun Day5a(lines: Flow<String>): Any {
  ...
  return pageLists.map { pages -> if (isValid(pages, rules)) pages[pages.size / 2] else 0 }.sum()
}
...
suspend fun Day5b(lines: Flow<String>): Any {
  ...
  return pageLists
      .map { pages -> if (isValid(pages, rules)) 0 else fixOrder(pages, rules)[pages.size / 2] }
      .sum()
}

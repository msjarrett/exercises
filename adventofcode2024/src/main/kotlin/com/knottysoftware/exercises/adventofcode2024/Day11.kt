...
// The array increases geometrically. Iteration 25 is 180k big, but Iteration 30 is 1500k big. Never
// making it to 75.
fun blinkUpdate(initialStones: List<Long>, iterations: Int): List<Long> {
  // Presumably with so many updates, MutableList should be more efficient than recreating immutable
  // lists.
...
private fun blinkCounterDigit(
  ...
  if (stone == 0L) {
    ...
    val strB = stoneString.substring((stoneString.length / 2)..<stoneString.length)
    result =
        blinkCounterDigit(strA.toLong(), iterations - 1, memo) +
            blinkCounterDigit(strB.toLong(), iterations - 1, memo)
  } else {
